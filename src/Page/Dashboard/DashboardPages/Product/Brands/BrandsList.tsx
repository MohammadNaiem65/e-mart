...
const columns = [
  {
    ...
    wrap: true,
    style: {
      width: "10%", // Set fixed width for Brand Name
      fontSize: "18px",
    },
  },
  {
    ...
    cell: (row) => (
      <img src={row.logo} alt={row.name} width="50" className="rounded-full" />
    ),
    wrap: true,
    style: {
      width: "20%", // Set fixed width for Brand Logo
    },
  },
  {
    name: "Brand Url",
    ...
    wrap: true,
    style: {
      width: "30%", // Set fixed width for Brand Url
    },
  },
  {
    name: "Status",
      ...
      <div className="form-control">
        <label className="label cursor-pointer">
          <input
            type="checkbox"
            className="toggle"
            defaultChecked={row.status === "active"} // Corrected syntax
            onChange={() => handleStatusChange(row.id, row.status)} // Optional handler
          />
        </label>
      </div>
    ),
    style: {
      width: "15%", // Adjusted width for the switch
    },
  },
  {
    name: "Actions",
...
const BrandsList = ({ data }) => {
  ...
  const filteredData = data.filter(
    (brand) =>
      ...
      brand.url.toLowerCase().includes(search.toLowerCase()) ||
      brand.status.toLowerCase().includes(search.toLowerCase())
  );

  // Calculate the data to display based on pagination and rows per page
  ...
  return (
    <div>
      {/* Render DataTable with responsive property */}
      <DataTable
        columns={columns}
        data={currentItems}
        selectableRows
